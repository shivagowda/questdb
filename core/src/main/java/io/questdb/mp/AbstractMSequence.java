...
abstract class AbstractMSequence extends AbstractSSequence {
    ...
    public void done(long cursor) {
        Unsafe.UNSAFE.putOrderedInt(flags, (((int) (cursor & mask)) << Unsafe.INT_SCALE) + Unsafe.INT_OFFSET, (int) (cursor >>> shift));
        barrier.getWaitStrategy().signal();
    }

    private boolean available0(long lo) {
        return Unsafe.UNSAFE.getIntVolatile(flags, (((int) (lo & mask)) << Unsafe.INT_SCALE) + Unsafe.INT_OFFSET) == (int) (lo >>> shift);
    }
}
